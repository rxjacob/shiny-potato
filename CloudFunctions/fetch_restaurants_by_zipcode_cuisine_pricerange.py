...
def get_restaurants_by_zipcode_cuisine_pricerange(request):
    ...
    stmt = sqlalchemy.text("select name from restaurant_9 where zip_code=:zip and category like :cuisine and price_range=:pricerange limit 10")

    text = "Here is a list of restaurants: "

    with connect_with_connector().connect() as db_conn:
        # read from database
        rows = db_conn.execute(stmt, parameters={"zip": zip_code, "cuisine": cuisine, "pricerange": price_range}).fetchall()
